document.addEventListener('DOMContentLoaded', async () => {
  ...
  const hideBrokenMedia = (root) => {
    (root || document).querySelectorAll('img').forEach(img => {
      ...
      img.addEventListener('error', () => {
        const wrap = img.closest('.card-image, .article-figure, .article-gallery');
        if (wrap) wrap.remove();
        const card = img.closest('.news-card');
        if (card && !card.querySelector('.card-image')) card.classList.add('is-text');
      });
    });
  };
  ...
  function renderArticle(item) {
    const id = item.id;
    const title = escapeHtml(cleanTitle(item));
    ...
    const media = image
      ...
      : '';
    if (!image && !isVideo) article.classList.add('is-text');
    article.innerHTML = `
      ${media}
      <div class="card-body">
        <div class="card-head">
