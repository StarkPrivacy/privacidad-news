document.addEventListener('DOMContentLoaded', async () => {
  ...
  const cleanTitle = (item) => {
    ...
    return rawTitle || rawExcerpt;
  };
  const cleanExcerpt = (item) => {
    const title = cleanTitle(item);
    const excerpt = stripLeadEmoji(stripUrls(item.excerpt || ''));
    if (!excerpt || normalize(excerpt) === normalize(title)) return '';
    return excerpt;
  };
  ...
  const hideBrokenMedia = (root) => {
    (root || document).querySelectorAll('img').forEach(img => {
      if (img.dataset.errBound) return;
      img.dataset.errBound = '1';
      const fail = () => {
        const cardWrap = img.closest('.card-image');
        if (cardWrap) {
          cardWrap.classList.add('is-fallback');
          if (!cardWrap.querySelector('.fallback-letter')) {
            const mark = document.createElement('span');
            mark.className = 'fallback-letter';
            mark.setAttribute('aria-hidden', 'true');
            const card = img.closest('.news-card');
            const item = card && articlesById[card.dataset.id];
            mark.textContent = item ? fallbackLetter(item) : 'P';
  ...
  function mediaBlock(item) {
    ...
    if (item.video_url) {
      return `<div class="native-player">
        <video class="article-video" controls playsinline preload="metadata" poster="${escapeHtml(item.video_thumb || item.image || '')}" src="${escapeHtml(item.video_url)}"></video>
    ...
    if (imgs.length === 1) {
      return `<figure class="article-figure"><img src="${escapeHtml(imgs[0])}" alt="" class="article-hero" data-full-image="${escapeHtml(imgs[0])}" /></figure>`;
    }
    return `<div class="article-gallery">${imgs.map(src =>
      `<figure class="article-figure"><img src="${escapeHtml(src)}" alt="" class="article-hero" data-full-image="${escapeHtml(src)}" /></figure>`
    ).join('')}</div>`;
  }
  ...
  function sourcesHtml(item) {
    ...
    return `<section class="article-sources"><h3>Fuentes</h3><ul>${
      sources.map(url => `<li><a href="${escapeHtml(url)}" target="_blank" rel="noopener">${escapeHtml(url)}</a></li>`).join('')
    }</ul></section>`;
  }
  ...
  function renderArticle(item) {
    ...
    const fullTitle = escapeHtml(cleanTitle(item));
    const excerpt = escapeHtml(cleanExcerpt(item));
    const dateLabel = formatDate(item.date);
    const isYt = Boolean(item.youtube_id);
    const isVideo = isYt || Boolean(item.video_url);
    ...
    const media = image
      ? `<div class="card-image${isVideo ? ' is-video' : ''}"><img src="${image}" alt="" loading="lazy" width="400" height="400" />${isVideo ? '<span class="play-badge" aria-hidden="true">▶</span>' : ''}</div>`
      : `<div class="card-image is-fallback" aria-hidden="true"><span class="fallback-letter">${escapeHtml(fallbackLetter(item))}</span></div>`;
    article.innerHTML = `
      ${media}
      <div class="card-body">
        <div class="card-head">
      ...
      <template class="full-content">
        <h1>${fullTitle}</h1>
        <p class="article-meta"><time datetime="${escapeHtml(item.date || '')}">${dateLabel}</time></p>
  ...
  function applyFilters(resetPage) {
    ...
    matches.slice(start, start + FEED_PAGE).forEach(card => { card.hidden = false; });
  ...
  function bindCardEvents() {
    ...
    document.querySelectorAll('.news-card .category, .tag-chip').forEach(badge => {
      badge.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        setActiveFilter(badge.dataset.filter || badge.textContent.trim().replace(/^#/, ''));
      });
    });
